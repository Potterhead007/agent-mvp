...
/// Strip characters that could inject additional env vars, comments, or corrupt the file.
...
/// Build a .env file body. Each key=value pair on its own line, no leading whitespace.
...
/// Read-only check: does ~/agent-mvp/docker-compose.yml exist?
...
/// Parse a .env file body into an ordered key→value map, skipping comments and blanks.
...
/// Serialize a key→value map into a .env file body with a header comment.
...
/// Write an env file with 0o600 permissions on Unix.
...
/// Sync credentials from the encrypted vault into the gateway .env files.
/// This merges new/changed keys into the existing .env without destroying
/// auto-generated secrets (POSTGRES_PASSWORD, REDIS_PASSWORD, GATEWAY_TOKEN).
...
// ---------------------------------------------------------------------------
// Direct env secret: write a single key-value pair to .env files without vault
// ---------------------------------------------------------------------------

/// Write a single key-value pair directly into the gateway .env files.
/// Used in web/bridge mode where there is no vault — credentials go straight
...
/// List all non-empty key names from the gateway .env file.
/// Returns a JSON array of objects with key names (no values for security).
...
/// Remove a key from the gateway .env file by setting its value to empty.
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Full roundtrip: sync -> read -> list -> remove -> verify
    // -----------------------------------------------------------------------
