...
fn resource_path(resource_dir: &str, filename: &str) -> Result<PathBuf, String> {
    let path = PathBuf::from(resource_dir).join(filename);
    if path.is_file() {
        return Ok(path);
    }
...
pub fn bootstrap_backend(
    ...
    let repo_root = Path::new(resource_dir)
        .parent()  // src-tauri
...
pub fn write_backend_env(
    ...
    write_env_secure(&root_env_path, &root_env)?;
    ...
    write_env_secure(&gw_env_path, &gateway_env)?;
        ...
        if let Ok(content) = fs::read_to_string(&config_path) {
            if let Ok(mut config) = serde_json::from_str::<serde_json::Value>(&content) {
                ...
                if let Ok(updated) = serde_json::to_string_pretty(&config) {
                    atomic_write_secure(&config_path, &updated)
...
fn serialize_env_map(map: &BTreeMap<String, String>) -> String {
...
pub fn sync_env_secret(state: &AppState, key: String, value: String) -> Result<(), String> {
    let openclaw_dir = Path::new(&state.openclaw_dir);
...
pub fn read_env_secret(state: &AppState, key: String) -> Result<Option<String>, String> {
    let path = Path::new(&state.openclaw_dir).join(".env");
    if !path.is_file() {
        return Ok(None);
    }
